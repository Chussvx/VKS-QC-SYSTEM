...
function submitCheckoutReport(reportData) {
  try {
    ...
    if (reportData.photoBase64) {
      const fileName = 'Checkout_' + reportData.guardId + '_' + new Date().getTime() + '.jpg';
      const uploadRes = Utils.uploadFileToDrive(reportData.photoBase64, fileName);
    ...
    // Map to QC Dashboard Scans columns: id, guardId, checkpointId, siteId, timestamp, lat, lng, accuracy, status, round
    const record = {
